...
@Component({
  ...
  standalone: true,
  imports: [CommonModule, FormsModule, RouterLink], // 補上 CommonModule 以支援 *ngFor/*ngIf
...
export class VolunteerBatchEditComponent implements OnInit {
 ...
 loadDataFromService(): void {
  ...
  try {
    const selectedDemands: VolunteerDemand[] =
      JSON.parse(storedData);

    this.editDemands = JSON.parse(
      JSON.stringify(selectedDemands)
    );
 ...
 saveAll(): void {
  // 找出有勾選的資料
  const selectedDemands = this.editDemands.filter(
