...
} from '../../../../models/agency/volunteer-demand';
...
} from '../../../filter/volunteer-filter/volunteer-filter.component';
import {
  VolunteerSortBarComponent,
  SortType,
} from '../../../sort-bar/volunteer-sort-bar/volunteer-sort-bar.component';
...
export class VolunteerListComponent implements OnInit {
  ...
  private readonly scrollPositionKey =
    'agency-disaster-workspace-scroll';
  private readonly pagePositionKey =
    'agency-disaster-workspace-page';
  ...
  statusOptions: DisplayVolunteerStatus[] = [
    '已上架',
    '隱藏中',
    '已下架',
  ];
  ...
  typeOptions: NonNullable<VolunteerDemand['type']>[] = [
    '物資搬運',
    '物資整理',
    '環境清潔',
    '醫療照護',
  ...
  async loadDemands(): Promise<void> {
    ...
    try {
      await this.volunteerDemandService
        .waitUntilLoaded();
      ...
      this.demands = this.volunteerDemandService
        .getDemands()
        .map((item) => ({
          ...item,
          selected: false,
          displayStatus:
            displayStatus[
              item.status as VolunteerStatus
            ],
          displayCreatedAt:
            item.status === '隱藏'
              ? '尚未發布'
  ...
  applyFilters(resetPage = true): void {
    this.filteredDemands = this.demands.filter(
      (item) => {
        if (
          this.searchTerm &&
  ...
  applySort(): void {
    this.filteredDemands = [
      ...
      if (this.selectedSort === 'createdAt') {
        result =
          this.getCreatedAtTime(a) -
          this.getCreatedAtTime(b);
      }
      ...
      if (
        ...
        const aTime = a.expectedOffShelfAt
          ? new Date(
              a.expectedOffShelfAt
            ).getTime()
          : 0;
        const bTime = b.expectedOffShelfAt
          ? new Date(
              b.expectedOffShelfAt
            ).getTime()
          : 0;
        result = aTime - bTime;
      }
  ...
  updatePagination(): void {
    ...
    this.selectAll =
      this.pagedDemands.length > 0 &&
      this.pagedDemands.every(
        (item) => item.selected
      );
  }
  ...
  changeStatus(
    ...
    const newStatus =
      select.value as DisplayVolunteerStatus;
    ...
    if (newStatus === '已下架') {
      ...
      if (originalItem?.status === '上架') {
        ...
        originalItem?.status === '隱藏'
      ) {
        item.displayStatus = '隱藏中';
      } else if (
        originalItem?.status === '下架'
      ) {
  ...
  private async applyStatusChange(
    ...
    if (status === '上架') {
      ...
      item.status = '上架';
      item.displayStatus = '已上架';
      item.displayPublishedAt = item.publishedAt
        ? new Date(
            item.publishedAt
          ).toLocaleDateString('zh-TW')
        : '尚未上架';
  ...
  calculateExpectedOffShelfDate(
    publishedDate: Date,
    priority: VolunteerDemand['priority']
  ): string {
    const offShelfDate = new Date(
      publishedDate
    );

    switch (priority) {
      case '普通':
        offShelfDate.setDate(
          offShelfDate.getDate() + 30
        );
        break;
      case '緊急':
        offShelfDate.setDate(
          offShelfDate.getDate() + 14
        );
        break;
      case '非常緊急':
        offShelfDate.setDate(
          offShelfDate.getDate() + 7
        );
        break;
    }
