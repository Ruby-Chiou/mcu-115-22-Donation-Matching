...
import {
  ...
  CreateDailyDemand,
  DailyConditions,
} from '../../../models/agency/daily-demand';
...
export class DailyDemandService {
  ...
  row.id
      ...
      priority:
        row.priority as DailyDemand['priority'],
      category:
        row.category as DailyDemand['category'],

      receiveMethod: {
        寄送: row.receiveMethod?.includes(
          '寄送'
        ) ?? false,

        面交: row.receiveMethod?.includes(
          '面交'
        ) ?? false,
      },
      ...
      customConditions:
        this.toStringArray(
          row.customConditions
        ),

      conditionDescription:
        this.createConditionDescription(
          row.conditions,
          row.customConditions
        ),

      status:
        row.status as DailyDemand['status'],
      ...
      contactTimeWeekday:
        row.contactTimeWeekday ?? false,

      contactTimeWeekend:
        row.contactTimeWeekend ?? false,
  ...
  const parsedSerialNo =
    Number(serialNo);

  if (
    serialNo !== null &&
    serialNo !== '' &&
    Number.isFinite(parsedSerialNo)
  ) {
  ...
  private demandToRow(
    ...
    return {
      createdAt:
        demand.createdAt || undefined,

      publishedAt:
        demand.publishedAt ?? null,
      ...
      customServiceTargets:
        demand.customServiceTargets ?? [],

      conditions:
        this.conditionsToArray(
          demand.conditions
        ),
      ...
      receiveMethod:
        this.receiveMethodToArray(
          demand.receiveMethod
        ),
      ...
      contactTimeWeekday:
        demand.contactTimeWeekday ?? false,

      contactTimeWeekend:
        demand.contactTimeWeekend ?? false,
  ...
  async updateDemand(
  updatedDemand: DailyDemand
): Promise<void> {
  if (updatedDemand.id == null) {
    throw new Error(
      `找不到資料庫 id，無法更新日常物資：serialNo=${updatedDemand.serialNo}`
    );
  }
  ...
  if (!data) {
    throw new Error(
      `找不到更新後的日常物資資料：id=${updatedDemand.id}`
    );
  }
  ...
  if (!data) {
    throw new Error(
      `找不到要更新的日常物資，serialNo：${serialNo}`
    );
  }

  const savedDemand = this.mapRowToDemand(
    data as DailyDemandRow
  );
  ...
  async deleteDemand(
    ...
    if (!data || data.length === 0) {
      throw new Error(
        `找不到要刪除的日常物資需求，serialNo：${serialNo}`
      );
    }
  ...
  for (const condition of value ?? []) {
    ...
    const name =
      condition.slice(
        0,
        separatorIndex
      ) as keyof DailyConditions;

    const result =
      condition.slice(
        separatorIndex + 1
      ) as DailyConditions[
        keyof DailyConditions
      ];
  ...
  private conditionsToArray(
    conditions: DailyDemand['conditions']
  ): string[] {
  ...
  private createConditionDescription(
    conditions: string[] | null,
    customConditions: unknown[] | null
  ): string {
    return [
  ...
  private receiveMethodToArray(
    receiveMethod: DailyDemand['receiveMethod']
  ): string[] {
